...
def validate_and_format_row(result: dict, claim_row: dict) -> dict:
    ...
    # Post-processing logical corrections to boost score and fix VLM inconsistencies
    ...
    # 6. Enforce issue type to severity mapping rules (Rule 17)
    # We prioritize the VLM's visual severity assessment if it is valid (low, medium, high).
    # If the VLM fails to output a valid severity or outputs none/unknown for a visible issue,
    # we fall back to a reasonable baseline mapping based on problem_statement.md definitions.
