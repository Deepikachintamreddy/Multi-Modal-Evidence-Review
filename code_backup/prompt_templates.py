...
9. CLOSE-UP CROP LENIENCY: Close-up shots of bumpers, doors, or corners often lack branding or model-identifying features. When evaluating multi-image claims, check for CONSISTENCY across images: (a) Does the damage location in the close-up correspond to the claimed part? (b) Is the body shape, panel curvature, or surface material consistent between images? (c) Is the background or setting similar? If the close-up shows the claimed damage on a plausible match for the claimed object, treat the claim as `supported` using the close-up image ID. Only flag `wrong_object` or `claim_mismatch` when images show genuinely incompatible objects (e.g., completely different vehicle body types like sedan vs. SUV, or different object categories). When in doubt about whether two images show the same object, lean toward evaluating the damage visible in the close-up rather than rejecting the claim over uncertain identity.
